...
def get_all_days(db_root, source_name):
    all_days = utils.get_subdirectories(os.path.join(db_root, source_name))
    all_days.sort()
    return all_days
...
def get_articles_per_batch(db_root, source_name, date_string):
    ...
    all_batch_times = utils.get_subdirectories(path)
    all_batches = []
...
def get_last_fetched_headlines(db_root, provider):
    provider_dir = os.path.join(db_root, provider)
